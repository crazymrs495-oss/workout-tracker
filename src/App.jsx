...
export default function WorkoutTracker() {
  ...
  const computeStreak = useCallback((pausedList) => {
    ...
    for (let i = 0; i < 400; i++) {
      ...
      const dayId = WEEKDAY_MAP[dow];
      const rec = hist.find((r) => r.dayId === dayId && r.date === dateStr);
      const anyDone = !!rec && Object.values(rec.logs || {}).some((sets) => Object.values(sets).some((s) => s.done));
      if (anyDone) count++;
  ...
  useEffect(() => { if (historyLoaded) computeStreak(pausedDates); }, [pausedDates, historyLoaded, computeStreak]);
  useEffect(() => {
    if (!loaded || !historyLoaded || activeDay !== todayId()) return;
    computeStreak(pausedDates);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [logs, loaded, historyLoaded]);
      ...
      <div className="sticky top-0 z-30 backdrop-blur px-4 pt-5 pb-3" style={{ backgroundColor: C.headerBg, borderBottom: `1px solid ${C.cardBorder}` }}>
        <div className="flex items-center justify-between mb-3 gap-2">
          <div className="flex items-center gap-2 shrink-0">
            ...
            <span className="text-[11px] tracking-[0.2em] uppercase font-bold" style={{ color: C.textFaint }}>Iron Log</span>
          </div>
        ...
        <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1" style={{ scrollbarWidth: "none" }}>
          {PROGRAM.map((d) => {
            ...
            return (
              ...
              >
                <div className="text-[9px] uppercase tracking-wider font-bold" style={{ color: active ? "#ffffff" : C.textFaint }}>{d.day.slice(0, 3)}</div>
                <div className="text-xs font-bold whitespace-nowrap" style={{ color: active ? "#ffffff" : C.text }}>{d.title}</div>
              </button>
            );
          })}
        </div>
      ...
      <div className="px-4 pt-3 flex items-center gap-2">
        ...
        {!sessionStart && (
          <button
            ...
            className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full font-bold transition active:scale-95"
            style={{ backgroundColor: C.accent, color: "#ffffff" }}
          >
            <Play size={12} color="#ffffff" fill="#ffffff" /> Start Workout
          </button>
        )}
